...
const ProductDetail = () => {
  ...
  useEffect(() => {
    ...
    const fetchRelated = async () => {
      const res = await fetch(apiUrl(`/api/products/${id}/related`));
      const data = await res.json();
  ...
  // Initialize selected color to first variant when product loads
  useEffect(() => {
    if (!product) return;
    const cvs = Array.isArray(product?.meta?.colorVariants) ? product.meta.colorVariants : [];
    if (cvs.length > 0) {
      setSelectedColor(0);
      setGalleryIndex(0);
    }
  ...
  // Color variants from meta
  const colorVariants = useMemo(() => {
    if (!product?.meta?.colorVariants) return [];
    return Array.isArray(product.meta.colorVariants) ? product.meta.colorVariants : [];
  }, [product]);

  // Gallery: use selected color's images if available, else fall back to flat images
  const gallery = useMemo(() => {
    if (!product) return [];
    if (selectedColor !== null && colorVariants[selectedColor]?.images?.length > 0) {
      return colorVariants[selectedColor].images.filter(Boolean);
    }
    const metaImages = Array.isArray(product?.meta?.images) ? product.meta.images : [];
    return [product.image_url, ...metaImages].filter(Boolean);
  }, [product, selectedColor, colorVariants]);

  // Auto-slide when color is selected
  useEffect(() => {
        ...
        <section className="relative container mx-auto px-4 lg:px-8 py-12">
          ...
          <div className="grid gap-10 lg:grid-cols-[1.1fr_1fr] relative z-10">
            ...
            <motion.div variants={scaleIn} initial="hidden" animate="visible">
                      ...
                      {gallery.map((_, idx) => (
                        <button
                          ...
                          className={`h-1.5 rounded-full transition-all duration-300 ${
                            idx === galleryIndex ? "w-5 bg-pink-500" : "w-1.5 bg-white/60"
                          }`}
                        />
                      ))}
                    </div>
              ...
              {/* Color Variant Thumbnails (shown below slider) */}
              {colorVariants.length > 0 && (
                ...
                >
                  <p className="text-xs font-semibold text-foreground/50 uppercase tracking-widest mb-3">
                    Color Variants
                  </p>
                  <div className="flex flex-wrap gap-3">
                    {colorVariants.map((cv, idx) => (
                      ...
                      >
                        <div
                          className={`h-16 w-20 overflow-hidden rounded-xl border-2 transition-all duration-300 shadow-sm ${
                            selectedColor === idx
                              ? "border-pink-500 ring-2 ring-pink-200 shadow-md"
                              : "border-gray-200 hover:border-yellow-400 opacity-70 hover:opacity-100"
                          }`}
                        >
                          <img
                            src={cv.images?.[0]}
                        ...
                        <span
                          className={`text-[11px] font-medium transition-colors ${
                            selectedColor === idx ? "text-pink-600" : "text-foreground/50"
                          }`}
                        >
                          {cv.name}
                        </span>
              ...
              {/* Flat thumbnails (used when no colorVariants) */}
              {colorVariants.length === 0 && gallery.length > 1 && (
                <motion.div
            ...
            >
              ...
              <motion.div variants={fadeUp} custom={2} className="grid gap-3 text-sm">
                ...
                {/* Color Variants selector in product info (right side) */}
                {colorVariants.length > 0 ? (
                  <div className="rounded-xl bg-gradient-to-r from-yellow-50 to-pink-50 px-4 py-3 border border-yellow-100/50">
                    <div className="flex items-center justify-between mb-3">
                      <span className="font-medium text-foreground/80">Color Variants</span>
                      {selectedColor !== null && (
                      ...
                      {colorVariants.map((cv, idx) => (
                        <button
                          ...
                          className={`flex items-center gap-2 px-3 py-1.5 rounded-full text-xs font-medium border transition-all duration-200 ${
                            selectedColor === idx
                              ? "border-pink-500 bg-pink-500 text-white shadow-sm shadow-pink-200"
                              : "border-gray-200 bg-white text-foreground/60 hover:border-pink-300 hover:text-pink-600"
                          }`}
                        >
                          {cv.images?.[0] && (
                            <img
